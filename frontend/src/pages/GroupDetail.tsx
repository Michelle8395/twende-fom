      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
        ...
        <div className="glass-card" style={{ padding: '16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ fontSize: '0.9rem', fontWeight: '700' }}>{club.nextEventDate ? new Date(club.nextEventDate).toLocaleDateString() : 'TBD'}</span>
            </div>
        </div>
      </div>
            ...
            {club.members.map((member: any) => (
                <div key={member.userId} className="glass-card flex-between" style={{ padding: '12px 16px', borderRadius: '14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                            <p style={{ fontSize: '0.9rem', color: 'var(--text-main)', fontWeight: '600' }}>{member.name} {member.userId === user.id && '(Me)'}</p>
                            <p style={{ fontSize: '0.7rem' }}>{member.role === 'admin' ? 'Organizer' : 'Contributor'}</p>
                        </div>
                    </div>
                    <span style={{ fontSize: '0.85rem', fontWeight: '700' }}>KES {member.contributed.toLocaleString()}</span>
                </div>
            ))}
        </div>
      </section>
